  ...
  try {
    ...
    const comitee = await prisma.comitee.create({
      data: {
        ...
        commiteeLeader: {
          connect: leaderIds && leaderIds.length > 0 ? leaderIds.map((id: string) => ({ id })) : [],
        },
        commiteeMember: {
          connect: memberIds && memberIds.length > 0 ? memberIds.map((id: string) => ({ id })) : [],
        },
    },
      include: {
        commiteeLeader: true,
...
router.get("/church/:churchId", async (req, res) => {
  try {
    ...
    if (error && typeof error === 'object') {
      ...
      if ('clientVersion' in error) errorInfo.clientVersion = error.clientVersion;
      if ('meta' in error) errorInfo.meta = error.meta;
    }
  ...
  try {
    ...
    const comitee = await prisma.comitee.update({
      ...
      data: {
        ...
        meetingTime,
        commiteeLeader: {
          set: leaderIds.map((id: string) => ({ id })),
        },
        commiteeMember: {
