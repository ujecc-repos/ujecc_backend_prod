...
router.post('/', upload.fields([
  ...
  try {
    ...
    const grooomCertificatePath = files?.grooomCertificate ? 
      `/uploads/${path.basename(files.grooomCertificate[0].path)}` : null;
    
    const brideCertificatePath = files?.brideCertificate ? 
      `/uploads/${path.basename(files.brideCertificate[0].path)}` : null;
    
    const weddingCertificatePath = files?.weddingCertificate ? 
      `/uploads/${path.basename(files.weddingCertificate[0].path)}` : null;
    
    const convert1 = moment(`${birthDate}`, 'YYYY-MM-DD', true);
    const convert2 = moment(`${goomBirthDate}`, 'YYYY-MM-DD', true);
    const convert3 = moment(`${weddingDate}`, 'YYYY-MM-DD', true);
...
router.put('/:id', upload.fields([
  ...
  try {
    ...
    const grooomCertificatePath = files?.grooomCertificate ? 
      `/uploads/${path.basename(files.grooomCertificate[0].path)}` : existingMarriage.grooomCertificate;
    
    const brideCertificatePath = files?.brideCertificate ? 
      `/uploads/${path.basename(files.brideCertificate[0].path)}` : existingMarriage.brideCertificate;
    
    const weddingCertificatePath = files?.weddingCertificate ? 
      `/uploads/${path.basename(files.weddingCertificate[0].path)}` : existingMarriage.weddingCertificate;

    // Build update data object with only provided fields
    const updateData: any = {};
    
    if (brideFullname !== undefined) updateData.brideFullname = brideFullname;
    if (birthDate !== undefined) updateData.birthDate = new Date(birthDate);
    if (groomFullname !== undefined) updateData.groomFullname = groomFullname;
    if (goomBirthDate !== undefined) updateData.goomBirthDate = new Date(goomBirthDate);
    if (weddingDate !== undefined) updateData.weddingDate = new Date(weddingDate);
    if (weddingLocation !== undefined) updateData.weddingLocation = weddingLocation;
    if (officiantName !== undefined) updateData.officiantName = officiantName;
    if (civilStateOfficer !== undefined) updateData.civilStateOfficer = civilStateOfficer;
    if (witnessSignature !== undefined) updateData.witnessSignature = witnessSignature;
