...
router.post('/', async (req, res) => {
  try {
    const isRecurrin = __.pick(req.body, ["isRecurring"])
    const rest = __.omit(req.body, ["isRecurring", "status", "frequency"]) as Irest
    const frequencyf = __.pick(req.body, ["frequency"]) as {frequency: string}
