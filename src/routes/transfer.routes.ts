...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const fromChurch = await prisma.church.findUnique({
    ...
    const toChurch = await prisma.church.findUnique({
      where: { id: toChurchId },
    });
...
router.get('/', async (req, res) => {
  try {
    const transfers = await prisma.transfert.findMany({
      include: {
        fromChurch: true,
        toChurch: true,
        member: true,
...
router.get('/:id', async (req, res) => {
  ...
  try {
    const transfer = await prisma.transfert.findUnique({
      where: { id },
      include: {
        fromChurch: true,
        toChurch: true,
