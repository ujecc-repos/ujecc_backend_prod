...
const ensureConversationMembership = async (conversationId, userId) => client_1.prisma.conversationParticipant.findUnique({
    where: { conversationId_userId: { conversationId, userId } },
});
const removeMessageAttachment = async (attachmentUrl) => {
    if (!attachmentUrl)
        return;
    const filename = path_1.default.basename(attachmentUrl);
    if (!filename)
        return;
    try {
        await promises_1.default.unlink(path_1.default.join(__dirname, '../../uploads', filename));
    }
    catch (error) {
...
router.post('/conversations/groups', async (req, res) => {
    try {
        ...
        const requestedParticipantIds = Array.isArray(req.body.participantIds)
            ? req.body.participantIds.filter((id) => typeof id === 'string')
            : [];
        ...
        const activeUsers = await client_1.prisma.user.findMany({
...
router.post('/conversations/:conversationId/participants', async (req, res) => {
    try {
        ...
        const participantIds = [...new Set(requestedParticipantIds)]
            .filter((id) => id !== req.user.id)
            .slice(0, 99);
        const group = await client_1.prisma.conversation.findFirst({
        ...
        const updatedGroup = await client_1.prisma.conversation.update({
...
router.delete('/conversations/:conversationId/participants/:participantId', async (req, res) => {
    try {
        ...
        const result = await client_1.prisma.conversationParticipant.deleteMany({
...
router.delete('/conversations/:conversationId', async (req, res) => {
    try {
        ...
        await client_1.prisma.conversation.delete({ where: { id: conversationId } });
...
router.patch('/conversations/:conversationId/messages/:messageId', async (req, res) => {
    try {
        ...
        const existingMessage = await client_1.prisma.message.findFirst({
...
router.delete('/conversations/:conversationId/messages/:messageId', async (req, res) => {
    try {
        ...
        await client_1.prisma.message.delete({ where: { id: messageId } });
