...
const VISITOR_STATUSES = ['Nouveau', 'À contacter', 'Contacté', 'Revenu'];
...
const cleanOptionalString = (value, maxLength = 191) => {
    if (typeof value !== 'string')
        return null;
    const cleaned = value.trim().slice(0, maxLength);
    return cleaned || null;
};
const cleanRequiredString = (value, maxLength = 191) => typeof value === 'string' ? value.trim().slice(0, maxLength) : '';
...
const cleanStatus = (value) => {
    ...
    return VISITOR_STATUSES.includes(status) ? status : 'Nouveau';
};
const createVisitorCode = async () => {
    for (let attempt = 0; attempt < 12; attempt += 1) {
        const code = `VIS-${Math.floor(100000 + Math.random() * 900000)}`;
        const existing = await client_1.prisma.visitor.findUnique({ where: { code }, select: { id: true } });
        if (!existing)
            return code;
    }
...
router.get('/stats', async (req, res) => {
    try {
        ...
        const [total, newThisMonth, followUp, affiliated] = await Promise.all([
            client_1.prisma.visitor.count({ where: { churchId } }),
            client_1.prisma.visitor.count({ where: { churchId, visitDate: { gte: monthStart } } }),
            client_1.prisma.visitor.count({ where: { churchId, status: { in: ['Nouveau', 'À contacter'] } } }),
            client_1.prisma.visitor.count({ where: { churchId, isAffiliated: true } }),
        ]);
...
router.get('/reports/export', async (req, res) => {
    try {
        ...
        const [church, visitors] = await Promise.all([
            client_1.prisma.church.findUnique({ where: { id: churchId }, select: { name: true } }),
            client_1.prisma.visitor.findMany({
                where,
        ...
        if (format === 'docx') {
            const tableRows = [
                new docx_1.TableRow({
                    tableHeader: true,
                    children: ['Code', 'Nom complet', 'Genre', 'Téléphone', 'Date', 'Statut', 'Affilié(e)'].map((text) => new docx_1.TableCell({ children: [new docx_1.Paragraph({ children: [new docx_1.TextRun({ text, bold: true, color: 'FFFFFF' })] })], shading: { fill: '0F766E' } })),
                }),
                ...visitors.map((visitor) => new docx_1.TableRow({ children: [
                        visitor.code,
                    ...
                    ].map((text) => new docx_1.TableCell({ children: [new docx_1.Paragraph(String(text))] })) })),
            ];
            const document = new docx_1.Document({ sections: [{ children: [
                            new docx_1.Paragraph({ alignment: docx_1.AlignmentType.CENTER, children: [new docx_1.TextRun({ text: `Rapport des visiteurs — ${churchName}`, bold: true, size: 34, color: '0F766E' })], spacing: { after: 180 } }),
                            new docx_1.Paragraph({ alignment: docx_1.AlignmentType.CENTER, children: [new docx_1.TextRun({ text: reportSubtitle, size: 20, color: '475569' })], spacing: { after: 100 } }),
                            new docx_1.Paragraph({ alignment: docx_1.AlignmentType.CENTER, children: [new docx_1.TextRun({ text: `Généré le ${generatedAt}`, italics: true, size: 18, color: '64748B' })], spacing: { after: 300 } }),
                            new docx_1.Table({ rows: tableRows, width: { size: 100, type: docx_1.WidthType.PERCENTAGE } }),
                        ] }] });
        ...
        const document = new pdfkit_1.default({ size: 'A4', layout: 'landscape', margin: 36, bufferPages: true });
        document.pipe(res);
        const drawHeader = () => {
        ...
        visitors.forEach((visitor, index) => {
            ...
            const y = document.y;
            if (index % 2 === 0)
                document.rect(36, y - 3, 770, 19).fill('#f0fdfa');
        ...
        if (!visitors.length)
            document.fillColor('#64748b').fontSize(11).text('Aucun visiteur ne correspond aux critères sélectionnés.', { align: 'center' });
        document.end();
    }
    catch (error) {
        console.error('Visitor report error:', error);
        if (!res.headersSent)
            res.status(500).json({ message: 'Impossible de générer le rapport des visiteurs' });
    }
});
router.get('/', async (req, res) => {
    try {
        ...
        const [items, total] = await client_1.prisma.$transaction([
            ...
            client_1.prisma.visitor.count({ where }),
        ]);
        res.json({
            items,
...
router.get('/:id', async (req, res) => {
    try {
        const visitor = await client_1.prisma.visitor.findFirst({
...
router.post('/', async (req, res) => {
    try {
        ...
        const visitor = await client_1.prisma.visitor.create({
            data: {
...
router.put('/:id', async (req, res) => {
    try {
        const existing = await client_1.prisma.visitor.findFirst({
            where: { id: req.params.id, churchId: req.visitorAdmin.churchId },
        ...
        const visitor = await client_1.prisma.visitor.update({
...
router.patch('/:id/status', async (req, res) => {
    try {
        ...
        if (!existing)
            return res.status(404).json({ message: 'Visiteur introuvable' });
        const visitor = await client_1.prisma.visitor.update({ where: { id: existing.id }, data: { status } });
        res.json(visitor);
    }
    catch (error) {
...
router.delete('/:id', async (req, res) => {
    try {
        const result = await client_1.prisma.visitor.deleteMany({
            where: { id: req.params.id, churchId: req.visitorAdmin.churchId },
        });
        if (!result.count)
            return res.status(404).json({ message: 'Visiteur introuvable' });
